...
local entities = {
    ...
    pc, couloir, bdabds, animest, tektn, bar
} --ordrer relevant
...
local ROOM_BG = {
    ...
    PC_BG, COULOIR_BG, BDABDS_BG, ANIMEST_BG, TEKTN_BG, BAR_BG
}

--screen scalling from 240*135
local S = 4

--inventory drawing constants
local INVENTORY = love.graphics.newImage("/assets/inventory.png")
local INV_X = 3
local INV_Y = 3
--local INV_W = INVENTORY:getWidth()
...
--local font = love.graphics.newFont("assets/LanaPixel.ttf", 3.5*S)
...
local jukebox = love.audio.newSource("assets/music/radio/1.mp3", "stream") -- janky inizialitaiion
...
local sound_ascensorcrash = love.audio.newSource("assets/music/ascenseur_crash.wav", "static")
local happy_bgm = love.audio.newSource("assets/music/happy_bgm.mp3", "stream")
local bgm = love.audio.newSource("assets/music/menu_bgm.ogg", "stream")
local sad_bgm = love.audio.newSource("assets/music/sad_bgm.mp3", "stream")
local upper_bgm = love.audio.newSource("assets/music/upper_bgm.mp3", "stream")
upper_bgm:setVolume(0.9)
local lab_bgm = love.audio.newSource("assets/music/lab_bgm.mp3", "stream")
local triggers = {0,0,0,0,0,0,0,0} -- {computer, note, screwd, battery_set, fuse_asc, nuclear door, cake, fullnote}
local chem_mix = {nil, nil, nil}
local chem_mix_color = {nil, nil, nil}
local chem_layerindex = 0
...
--following complete item definitions from other files
    ...
    if item.interact_with ~= nil then
        for i=1, #item.interact_with do
            if item.interact_with[i] == item_selected.id then
                ...
                --switch on triggers
                if item.trigger ~= nil then
                    triggers[item.trigger] = 1
                end

                fuse_switch(item.interact_fuse, 0)

                --mix potion
                if (item_selected.id == "pot_empty") and item.chem_clear_cyl and (chem_layerindex > 0) then
                    local nb, nr, ng, ns = 0, 0, 0, 0
                    for k=1, chem_layerindex do
                        local color = chem_mix_color[k]
                        if color == "red" then
                            nr = nr + 1
                    ...
                    if nr == 2 and nb ==1 then
                        table.insert(inventory, item.chem_pot_pool[1])
                    elseif nr == 1 and nb == 1 and ns == 1 then
                        table.insert(inventory, item.chem_pot_pool[4])
                    elseif ng == 2 and ns == 1 then
                        table.insert(inventory, item.chem_pot_pool[2])
                    else
                        table.insert(inventory, item.chem_pot_pool[3])
                    end
                    table.remove(inventory, item_selected.i)
                end

                local chem_color= item.chem_color
                if chem_color ~= nil and chem_color[i] ~= nil  and chem_layerindex < 3 then
                    chem_layerindex = chem_layerindex + 1
                    chem_mix[chem_layerindex] = item.chem_img[i]
                    chem_mix_color[chem_layerindex] = chem_color[i]
                end
                ...
                if item.interact_clear_mix and item_selected.id == "pot_empty" then
                    chem_mix = {nil, nil, nil}
                    chem_mix_color = {nil, nil, nil}
                    chem_layerindex = 0
                    text_queue(1, "You filled the potion.")
                end
            end
        end
    end
...
local function drag(i)
    local item = item_room[i]
    if item ~= 0 and item.cursor ~= nil and dragged == nil then
        do_clear_cursor = false
        dragged = item_room[i]
        item_selected = {}
        item_selected.id = dragged.id
        item_room[i] = 0
        love.mouse.setCursor(dragged.cursor)
    end
end
...
local function toggle_screen()
    ...
    if fullscreen_state then
        S = 4
        font = love.graphics.newFont("assets/LanaPixel.ttf", 3.5*S)
    else
        S = 8
        font = love.graphics.newFont("assets/LanaPixel.ttf", 3*S)
    end
end
...
local function sensors()
    ...
    for i=1, #item_room do
        ...
        if item ~= 0 and item ~= nil and cursor_in_retangle(item.x, item.y, item.w, item.h) then
            ...
            if item.set_bgm ~= nil then
                if item.set_bgm == "happy" then
                    bgm:stop()
                    audio_pause = false
                    bgm = happy_bgm
                end
            ...
            --rearrange end room for good ending
            if item.room ~= nil then
                entities[7] = item.room[1]
                entities[9] = item.room[2]
                get_item_dim()
            end
            ...
            if item_room[i] ~= 0 and item.dialog ~= nil and item_selected.id == nil then
                if (item.condition == "fuse" and dark_room[7] == 1 and triggers[6] == 0) then
                    dialog(item, i)
                    item_room[4] = 0 --remove first text
                    table.insert(inventory, item.inventory)
                    break
                elseif (item.condition == "white_room"  and triggers[6] == 0 and dark_room[7] == 0) then
                    print("white_room_trigger")
                    dialog(item, i)
                    break
                elseif item.dialog_repeat == true then
                    table.insert(item_room, item)
                    dialog(item, i)
                    break
            ...
            --clear cylinder
            if ((item.chem_clear_cyl and dragged == nil and item_selected.id == nil)
            or item_selected == "pot_empty" ) and chem_layerindex > 0 then
                chem_mix = {nil, nil, nil}
                chem_mix_color = {nil, nil, nil}
                chem_layerindex = 0
                text_queue(1, "You emptied the cylinder in the drain.")
            end
            ...
            --trigger verif
            local trigger_vf = item.trigger_verif
            if trigger_vf ~= nil then
                if trigger_vf == "lab" and triggers[5] == 1 and triggers[4] == 1 and triggers[6] == 0 then
                    triggers[5] = 0 --so it dont double launch dialog
                    table.insert(item_room, item.trigger_stuff)
                    audio_pause = true
                    bgm:pause()
                    get_item_dim()
                elseif trigger_vf == "lab" and item.notrigger_text ~= nil then
                    text_queue(2, item.notrigger_text)
                elseif trigger_vf == "go_nuke" and triggers[6] == 1 then
                    current_room = 9
                    item_room = entities[9]
                    text_buffer = nil
                    audio_pause = false
                    bgm:stop()
                    bgm = sad_bgm
                    break
                elseif trigger_vf == "asc" and triggers[1] == 1 and triggers[2] == 1 and triggers[3] == 1 then
                    current_room = 5
                    item_room = entities[5]
                    text_buffer = nil --to not show ascensor text if crossing
                    sound_ascensorcrash:play()
                    bgm:pause()
                    audio_pause = true
                    dialog(item_room[#item_room], #item_room)
                    break
                elseif trigger_vf == "ending" and triggers[7] == 1 and triggers[8] == 1 then
                    text_buffer = nil
                    inventory = {}
                    item_room = {item.end_pic}
                    Timer.after(3, function()
                        item_room = {item.end_pic2}
                    end)
                    break
                end
            end
            ...
            --tableau
            if item.id == "tableau" then
                item.x = 58
                item.y = 0
                bgm:pause()
                jukebox = item.music
                jukebox:play()
                item.img = item.screamer
                Timer.after(1, function()
                    entities[4][i].img = nil
                    entities[4][i].x = 95
                    entities[4][i].y = 23
                end)
            end
...
local function clear_cursor()
    if do_clear_cursor then
        ...
        table.insert(item_room, dragged) --maybe reput the object more elegantly at the right place
        dragged = nil
        item_selected = {}
        love.mouse.setCursor(CURSOR)
        do_clear_cursor = false
...
--    if current_room == 10 then
--        if chem_mix[1] ~= nil then
--            draw_S(chem_mix[1], 137, 93)
--        end
--        if chem_mix[2] ~= nil then
--            draw_S(chem_mix[2], 137, 85)
--        end
--        if chem_mix[3] ~= nil then
--            draw_S(chem_mix[3], 137, 77)
--        end
--    end
--end
